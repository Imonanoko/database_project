...
pub async fn search_allorder_data(
    data: web::Json<Data>,
    session: Session,
    db: web::Data<Pool>,
) -> HttpResponse {
        ...
        Ok(Some(is_admin)) => {
            if is_admin {
                ...
                let result: Vec<SendData> = conn
                        .exec_map(
                        "SELECT c.id_number, c.phone_number, COALESCE(SUM(o.order_amount), 0.0) AS total_order_amount \
                        ...
                        |row: mysql::Row| {
                            let id: String = row.get("id_number").unwrap();
